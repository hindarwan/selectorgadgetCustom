...
SelectorGadget.prototype.makeBorders = function(orig_elem, makeRed) {
    ...
    this.b_top.css('width', this.px(width + this.border_padding * 2 + this.border_width * 2 - 4)).//-4
            css('top', this.px(top - this.border_width - this.border_padding + 2)). //+2
            css('left', this.px(left - this.border_padding - this.border_width + 2));//+2
    this.b_bottom.css('width', this.px(width + this.border_padding * 2 + this.border_width * 2 - 5 - 4)).//-4
            css('top', this.px(top + height + this.border_padding - 2)). //-2
            css('left', this.px(left - this.border_padding - this.border_width + 2));//+2
//            css('left', this.px(left - this.border_padding - this.border_width + 2)).text(path_to_show);//+2
...
//            css('left', this.px(left - this.border_padding - this.border_width));
    ...
    this.b_right.css('height', this.px(height + this.border_padding * 2)).
            css('top', this.px(top - this.border_padding)).
            css('left', this.px(left + width + this.border_padding - 2));//-2
...
SelectorGadget.prototype.setupBorders = function() {
    if (!this.b_top) {
        ...
        this.b_bottom = jQuery('<div>').addClass('sg_border').addClass('sg_bottom_border').css('height', this.px(this.border_width)).hide().bind("mousedown.sg", {'self': this}, this.sgMousedown);
...
SelectorGadget.prototype.makeInterface = function() {
    ...
    this.clear_button = jQuery('<input type="button" value="Select an element and choose suitable property"/>').addClass('sg_ignore');
...
//            jQuery(el).attr('jfxid', x++);
    ...
    //TODO //critical
    //ignore selection
    jQuery('table, tbody, thead, tfoot, ul, ol').addClass('sg_ignore');
//    jQuery('td > *').addClass('sg_ignore');
//    jQuery('th > *').addClass('sg_ignore');
    
    //div :) not handle
//    jQuery('*').parent('div').addClass('sg_ignore');
//    jQuery('div').filter(function() {
//        return this.nodeType != 3;
//    }).addClass('sg_ignore');
//    jQuery('*').filter(function() {
//        return this.nodeType != 3;
//    }).parent('div').addClass('sg_ignore');
//    jQuery('div').addClass('sg_ignore');
